<?php
    ...
    if($_GET['id'] != null)
    {
       $query1 = $con->db->prepare("SELECT * FROM `XBOXIPHISTORY` WHERE `id` = :license");
       ...
       $result2 = $query1->fetch(PDO::FETCH_ASSOC);
        if($result2)
        {
            ...
            {
              $query = $con->db->prepare("DELETE FROM `XBOX_IPHISTORY` WHERE `id` = :license");
              $query->execute(array("license"=>$_GET['id']));
              header('Location: xbox_history.php');
            }
        }
        else
        {
            if($_GET['id'] == "all")
            {
              $query2 = $con->db->prepare("DELETE FROM `XBOX_IPHISTORY` WHERE `userid` = :license");
              $query2->execute(array("license"=>$_SESSION['id']));
              header('Location: xbox_history.php');
            }
        }
    }
    if(isset($_POST['XBOX_IPHISTORY'])){
        $con->update("XBOX_IPHISTORY", array("level"=>$_POST['1'],"prestige"=>$_POST['2'],"name"=>$_POST['3'],"ip"=>$_POST['4'], "port"=>$_POST['5'], "xuid"=>$_POST['6'], "game"=>$_POST['7']), "id", $id);
    }  
?>
<!DOCTYPE html>
<html lang="en">
                            ...
                            <table id="data-table" class="table table-sm">
                                <thead>
                                        <tr>         
                                            ...
                                            <tbody>
                                            <?php 
                                            $query = $con->db->prepare("SELECT * FROM `XBOXIPHISTORY` WHERE userid = :id ORDER BY `id` DESC");
                                            ...
                                            foreach($res as $row){
                                                ...
                                                echo '
                                                        <td>
                                                            ...
                                                            <a type="submit" value="Submit" class="btn btn-danger btn-sm pull-right" name="delete1" href="xbox_history.php?id='.$row['id'].'">Delete</a>    
                                                        </td>
                                                    </tr>
                                                ';
                                            }
