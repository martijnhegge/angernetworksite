<?php
    ...
    {
        $query1 = $con->db->prepare("INSERT INTO `blacklist` (`userid`,`username`,`name`,`ip`,`network`,`value`) VALUES (:userid, :username, :name, :ip, :game, 'value')");
        $query1->execute(array('userid'=>$_SESSION['id'],'username'=>$user->getFromTable_MyId("username", "users"),'name'=> $_POST['name'],'ip'=> $_POST['ip'],'game'=> $_POST['network']));
    }
        ...
        <header class="header">
            ...
            <div class="row"> 
                <div class="col-md-4">
                        ...
                        <div class="card-body">
                            ...
                            <form  method="POST">  
                                <div class="form-group"> 
                                    <input class="form-control text-center" name="name" type="text" placeholder="name"  disabled/>
                                    </div>
                                    <div class="form-group"> 
                                    <input class="form-control text-center" name="ip" type="text" placeholder="ip"  disabled/>
                                    </div>
                                    <div class="form-group">
                                    <select id="network" class="btn btn-theme btn-block grey dropdown-toggle" name="network" hidden>
                                        <option value="PlayStation" selected="selected" />PlayStation
                                    </select>
                                </div>
                                    <div class="form-actions">
                                    <button type="submit" name="blacklist" id="blacklist" class="btn btn-primary btn-block" disabled>Blacklist</button>
                                </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">your blacklists</h4>
                        <h6 class="card-subtitle"></h6>
                        ...
                        <div class="table-responsive data-table">
                            <table id="data-table" class="table table-sm">
                                ...
                                <tbody>
                                <?php 
                                $queryb = $con->db->prepare("SELECT * FROM `blacklist` WHERE userid = :id ORDER BY `id` DESC");
                                $queryb->execute(array("id"=>$_SESSION['id']));
                                $resb = $queryb->fetchAll();
                                ...
                                <td>'.$rowb['time'].'</td>
                                <td>
                                <button type="submit" class="btn btn-danger btn-block" name="removeblacklist" id="removeblacklist">Remove</button>
