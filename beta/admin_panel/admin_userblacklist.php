<?php
    ...
    if($_GET['id'] != null)
    {
       $query1 = $con->db->prepare("SELECT * FROM `black_list` WHERE `ID` = :license");
       ...
       $result2 = $query1->fetch(PDO::FETCH_ASSOC);
        if($result2)
        {
            ...
            {
              $query = $con->db->prepare("DELETE FROM `black_list` WHERE `ID` = :license");
              $query->execute(array("license"=>$_GET['id']));
              header('Location: admin_userblacklist.php');
            }
        }
        else
        {
            if($_GET['id'] == "all")
            {
              $query2 = $con->db->prepare("DELETE FROM `black_list` WHERE `userid` = :license");
                ...
                <div class="row">
                 <div class="col-md-4">
                       <?php
                        ...
                        {
                        if (!empty($_POST['typeip']))
                        {                 
                        $typeip = $_POST['typeip'];                                      
                        {  
                            $query1 = $con->db->prepare("UPDATE `black_list` SET `gamertag` = :gamertag WHERE `userid` = :id");
                            $query1->execute(array('id'=>$_SESSION['id'],'gamertag'=> $typeip));
                             }                            
                             echo '<div class="alert alert-success"><p>Added to Blacklist</p></div>';                           
                                }
                                else
                                {
                                ...
                                if (!empty($deletes))
                                {
                                 $query = $con->db->prepare("DELETE FROM `black_list` WHERE `ID` = :id AND `userid` = :uid LIMIT 1");
                    ...
                    <div class="card">
                    <div class="card-body">
                            <h4 class="card-title">Added Blacklistening</h4>
                            <h6 class="card-subtitle"></h6>
                            <form  method="POST">
                                        <div class="form-group">
                                        <input class="form-control text-center"name="typeip" value="" type="text" placeholder="Gamertag" />
                                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive data-table">
                            <table id="data-table" class="table table-sm">
                                ...
                                <tbody>
                                <?php 
                                $query = $con->db->prepare("SELECT * FROM `black_list` WHERE userid = :id ORDER BY `id` DESC");
                                ...
                                echo '
                                <td>
                                <a type="submit" class="btn btn-primary btn-block pull-right" name="deleteFromSafe" href="admin_userblacklist.php?id='.$row['ID'].'">Remove</a>
