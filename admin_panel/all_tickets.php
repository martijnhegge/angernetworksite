<?php
    ...
    if($did)
    {
        $query = $con->db->prepare("DELETE FROM `tickets` WHERE `id` = :id");
        $query->execute(array('id' => $did));
        $query2 = $con->db->prepare("DELETE FROM `ticket_messages` WHERE `ticketid` = :id");
        ...
        <header class="header">
            ...
            <section class="content">
                <header class="content__title">
                    <h1>All User Overview<small></small></h1>
                </header>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive data-table">
                            <table id="data-table" class="table table-sm">
                                ...
                                <tbody>
                                <?php 
                                            $query = $con->db->prepare("SELECT * FROM `tickets`");
                                            $query->execute();
                                            $res = $query->fetchAll();
                                            foreach($res as $row){
                                                echo '
                                                ...
                                                echo '
                                                        <td>
                                                            <a type="submit" class="btn btn-info btn-block" href="view_support.php?id='.$row['id'].'">Manage</a>
                                                        </td>
                                                        <td>
                                                            <a type="submit" class="btn btn-danger btn-block" href="all_tickets.php?id='.$row['id'].'">Delete</a>
                                                        </td>
                                                    </tr>
                                                ';
                                            }
