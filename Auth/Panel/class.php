<?php
	...
	class auth extends database{
		...
		public function LoginLogsUser($username,$password,$hwid)
		{
            $query = $this->db->prepare("SELECT * FROM users WHERE username = :license");
			$query->execute(array("license"=>$this->Decrypt($username)));
			$result = $query->fetch(PDO::FETCH_ASSOC);
			if($result)
			{
				if(password_verify($this->Decrypt($password),$result['password']))
				{
					if($result['hwid'] == $this->Decrypt($hwid))
					{
				        ...
				        {
				        	$query = $this->db->prepare("SELECT * FROM `login_logs` WHERE `userid` = :license");
	                        $query->execute(array("license"=>self::sanitize($result['id'])));
	                        $shit = $query->fetch(PDO::FETCH_ASSOC);
	                        $list = array();
	                        foreach ($shit as $row)
	                        {
	                        	array_push($list,$row["username"],$row["userid"]);
	                        }
		...
		public function LoginLogsUserTest($username,$password)
		{
            $query = $this->db->prepare("SELECT * FROM `users` WHERE `username` = :license");
			$query->execute(array("license"=>$username));
			$result = $query->fetch(PDO::FETCH_ASSOC);
			if($result)
			{
				if(password_verify($password,$result['password']))
				{
					...
					{*/
						$query = $this->db->prepare("SELECT * FROM `bans` WHERE `userid` = :license");
			            $query->execute(array("license"=>$result['id']));
			            $result2 = $query->fetch(PDO::FETCH_ASSOC);
			            if($result2)
			            {
				            if($result2['type'] == "Temp")
				            {
					            if(strtotime($result2['unban_when']) <= strtotime(date('Y-m-d h:i:s')))
					            {
						            $query = $this->db->prepare("DELETE FROM `bans` WHERE `username` = :license");
						            $query->execute(array("license"=>$username));
						            return ($this->Encrypt('{"result":"unbanned","sig":"'.$result['sig'].'"}'));
					            }
					            else
					            {
						            return ($this->Encrypt('{"result":"temp_banned","sig":"'.$result['sig'].'"}'));
					            }
				            }
				            else
				            {
				             	return ($this->Encrypt('{"result":"permanently_banned","sig":"'.$result['sig'].'"}'));   
				            }
				        }
				        else
				        {
				...
				{
                    return ($this->Encrypt('{"result":"password_invalid"}'));
				}
			}
			else
			{
				return ($this->Encrypt('{"result":"username_invalid"}'));
			}
		}
		...
		public function Login($username,$password, $hwid,$hash,$pcname){
			$canlogin = $this->db->prepare("SELECT * FROM `websiteSettings`");
			$canlogin->execute();
			$resultfm = $canlogin->fetch(PDO::FETCH_ASSOC);
			if($resultfm['2'] == '0')
			{
				die($this->Encrypt('{"result":"login_disabled","sig":"'.$result['sig'].'"}'));
			}
			$query = $this->db->prepare("SELECT * FROM users WHERE username = :license");
			$query->execute(array("license"=>$this->Decrypt($username)));
			$result = $query->fetch(PDO::FETCH_ASSOC);
			if($result)
			{
				...
				{
					die($this->Encrypt('{"result":"not_verfied","sig":"'.$result['sig'].'"}'));
				}
				if(password_verify($this->Decrypt($password),$result['password']))
				{
					if($result['hwid'] == $this->Decrypt($hwid))
